...
void Analyzer::writing_file_to_var() {//переводит из файла в перенную text
...
bool Analyzer::Checking_for_initialization_var(const string& var, const int& count_var) {
	...
	while (text[index] != '\0') {
		...
		bool flag = find_var(count_var, index_2, var);//поиск переменной

		if (flag) {
			index_2 = 1;
			while (text[index - index_2] == ' ') {
				index_2++;//пропускаем пробелы перед переменной
			}
			//while ((text[index - index_2] != ' ') && (text[index - index_2] != '\n') && (text[index - index_2] != ';') && (text[index - index_2] != '{') && (text[index - index_2] != '}')) {
			//	index_2++;//переходим в начало строки
			//}
			...
			index_2--;//переходим на первый символ типа
			...
			for (int i = 0; i < 4; i++) {
				...
				for (int j = 0; j < count_types[i]; j++, index_2--) {
					if (text[index - index_2] != types[i][j]) {//сравниваем типы
						flag_types = false;
						break;
					}
					if (flag_types == false) break;
				}
				if ((flag_types) && (text[index - index_2] != ' ')) return false;//проверка на пробел после int
...
bool Analyzer::find_var(const int& count_var, int& index_2, const string var) {
	bool flag = true;
	for (int i = 0; i < count_var; i++, index_2++) {//ищем переменную
	...
	if (flag) {
		flag = is_function(index_2);//проверяем не функция ли это
	}

	if ((flag) && ((text[index_2 - count_var - 1] != ' ') || ((text[index_2] != ' ') && (text[index_2] != '=') && (text[index_2] != ';')))) {
		flag = false;//проверка слева и справа от переменной
	}
...
bool Analyzer::is_function(int index) {
	while (text[index] == ' ') {//пропускаем пробелы после переменной
		index++;
	}
	if (text[index] == '(') {
		return false;
...
int Analyzer::main_change(int& count) {
	...
	if (!Checking_for_comments(count, should_change)) return 0;//проверка на комменты
	if (!Checking_for_quotation_marks(count, should_change)) return 0;//проверка на кавычки
	int count_var = 0;//количество символов переменной
	int count_whitespace = 0;//количество пробелов
	char sign = text[count];

	string var = "\0";
	int i = 1;
	while (text[count - i] == ' ') {//пропускаем пробелы перед знаком
		i++;
		count_whitespace++;
	}
	//while ((text[count - i] != ' ') && (text[count - i] != '\n') && (text[count - i] != ';') && (text[count - i] != '{') && (text[count - i] != '}')) {//идем до начала строки
	//	i++;
	//}
	...
	i--;//возвращаемся на первый символ переменной
	while ((text[count - i] != ' ') && (text[count - i] != sign)) {//считываем левую переменную
		var.append(1, text[count - i]);
		count_var++;
		i--;
	}

	bool get_init = Checking_for_initialization_var(var, count_var);//проверяем инициализацию левой переменной
	...
	while ((text[i] != ' ') && (text[i] != ';')) {//считываем вторую переменную
	...
	if (!get_init2) {
		get_init2 = Checking_for_initialization_var(var2, count_var2);//проверяем инициализацию правой переменной
	}
	...
	if ((get_init) && (get_init2)) {
		...
		text.erase(count - count_whitespace, count_whitespace);//удаляем пробелы после знака
		...
		text.erase(count2 + count_var + 2, count_whitespace);//удаляем пробелы справа
	}
	else {
		cout << endl << "Ошибки: " << endl;
		if (!get_init2)
			cout << "Не инициализирована переменная " << var2 << endl;
		if (!get_init)
			cout << "Не инициализирована переменная " << var << endl;
	}
...
bool Analyzer::Checking_for_quotation_marks(int& count, bool& shouold_change) {
	...
	while (text[i] != '\n') {//ищем кавычки на этой строке слева
		if (text[i] == '"') {
	...
	i = count;
	while (text[i] != '\n') { //ищем кавычки на этой строке справа
		if (text[i] == '"') {
			count_quotation_marks++;
			break;
		}
...
void Analyzer::print_text() {
	cout << endl << "Исправленный код: " << endl;
	cout << text << endl;
}
...
void Analyzer::writing_to_a_file() {
	...
	if (!out) {
		cout << "Файл с кодом не может быть исправлен!" << endl;
		getch();
		exit(1);
	}
	out << text;
